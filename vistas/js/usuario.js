  ...
  .done(function (respuesta) {
    ...
    for (var i = 0; i < respuesta.length; i++) {
      if (respuesta[i].per_Activo == 1) {
        $("#usuarioPERFIL").append(
          "<option value=" +
            respuesta[i].per_Id +
            ">" +
            respuesta[i].per_Nombre +
            "</option>",
        );
      }
    }
...
$("#modal-nuevo-usuario").on("click", ".btn-guardar-usuario", function (event) {
  ...
  if (user_Cedula === "" && !expr.test(user_Cedula)) {
    mensajesError.push(
      "Error Cedula: Revisar Campo debe contener un caracter no permitido o esta vacio",
    );
  }
  ...
  if (user_Password === "" && !expr.test(user_Password)) {
    mensajesError.push(
      "Error Contraseña: Revisar Campo debe contener un caracter no permitido o esta vacio",
    );
  }
  ...
  if (user_Nombre === "" && !expr.test(user_Nombre)) {
    mensajesError.push(
      "Error Nombre: Revisar Campo debe contener un caracter no permitido o esta vacio",
    );
  }
  ...
  if (user_Celular === "" && !expr.test(user_Celular)) {
    mensajesError.push(
      "Error Celular: Revisar Campo debe contener un caracter no permitido o esta vacio",
    );
  }
