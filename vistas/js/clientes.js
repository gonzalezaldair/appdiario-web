    ...
    if (respuesta.length > 0) {
      ...
      for (var i = 0; i < respuesta.length; i++) {
        if (respuesta[i].frm_Activo == 1) {
          $("#comboformapago").append(
            "<option value=" +
              respuesta[i].frm_Id +
              ">" +
              respuesta[i].frm_Nombre +
              "</option>",
          );
        }
      }
      ...
      $("#comboformapago")
        .parent()
        .after(
          '<div class="alert alert-warning">Debe agregar Formas de Pago. <a href="forma-pago">Ir a Formas de Pago</a></div>',
        );
    }
  })
    ...
    $.ajax({
      ...
      .done(function (respuesta) {
        ...
        if (respuesta.mensaje === "ok") {
          ...
          Swal.fire("Advertencia !", "Error: " + respuesta.codigo, "warning");
        }
      })
      .fail(function (respuesta) {
        console.log("respuesta", respuesta.responseText);
    ...
    .done(function (respuesta) {
      ...
      if (respuesta) {
        $("#clienteCelular")
          .parent()
          .after(
            '<div class="alert alert-warning">Este Telefono ya existe en la base de datos</div>',
          );
