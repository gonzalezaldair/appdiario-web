...
class PrestamosControlador{
	...
	public static function ctrguardarPrestamo()
	{
		...
		if (isset($_POST)) {
			...
			if (preg_match('/^[,\\.\\a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["pre_Observaciones"]) && preg_match('/^[0-9]+$/', $_POST["pre_CLIENTE"]) && preg_match('/^[0-9]+$/', $_POST["pre_FormaPago"]) && preg_match('/^[0-9]+$/', $_POST["pre_Interes"]) && preg_match('/^[0-9]+$/', $_POST["pre_MontoPrestado"]) && preg_match('/^[0-9]+$/', $_POST["pre_Cuotas"]))
			{
				$pre_Id = intval($_POST["pre_Id"]);
				$pre_Fecha = date('Y-m-d h:m:s');
